...
from utils import (rouge_score, bleu_score, Batchify, now_time,
                   ids2tokens, unique_sentence_percent, feature_detect,
                   feature_matching_ratio, feature_coverage_ratio, feature_diversity)

parser = argparse.ArgumentParser(description='PErsonalized Prompt Learning for Explainable Recommendation (PEPLER)')
parser.add_argument('--data_path', type=str, default=None,
                    help='path for loading the explain data')
parser.add_argument('--dict_path', type=str, default=None,
                    help='path for loading the user and item index dict data')
...
parser.add_argument('--output_path', type=str, default='generated.txt',
...
eos = '<eos>'
pad = '<pad>'
tokenizer = GPT2Tokenizer.from_pretrained('/Users/liuqiang/Desktop/code/llm/models/gpt2',
                                          bos_token=bos, eos_token=eos, pad_token=pad)
f = open(args.data_path, "rb")
corpus = json.load(f)
f = open(args.dict_path, "rb")
dic = json.load(f)
feature_set = set()
train = corpus['train']
val = corpus['val']
test = corpus['test']
feature_set = feature_set.union([d['feature'] for d in train])
feature_set = feature_set.union([d['feature'] for d in val])
feature_set = feature_set.union([d['feature'] for d in test])
...
# Build the model
###############################################################################
nuser = len(dic['user_dict'])
nitem = len(dic['item_dict'])
ntoken = len(tokenizer)
model = ContinuousPromptLearning.from_pretrained('/Users/liuqiang/Desktop/code/llm/models/gpt2', nuser, nitem)
...
def train(data):
    ...
    while True:
        ...
        if data.step % args.log_interval == 0 or data.step == data.total_step:
            ...
            print(now_time() + 'text ppl {:4.4f} | {}/{} batches'.format(math.exp(cur_t_loss),
...
def generate(data):
    ...
    with torch.no_grad():
        while True:
            user, item, seq, _ = data.next_batch()  # data.step += 1
            user = user.to(device)  # (batch_size,)
            item = item.to(device)
...
text_out = []
for (real, predicted) in zip(text_test, text_predict):
    sample_predict = {
        "real": real,
        "predicted": predicted
    }
    text_out.append(sample_predict)
with open(prediction_path, 'w', encoding='utf-8') as f:
    res = json.dumps(text_out, indent=4, ensure_ascii=False)
    f.write(res)
