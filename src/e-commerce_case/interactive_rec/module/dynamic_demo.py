...
from utils.util import replace_substrings_regex
...
class DemoSelector:
    ...
    def __init__(self, mode: str, demo_dir_or_file: str, k: int, domain: str) -> None:
        assert mode in {'fixed',
                        'dynamic'}, f"Optional demonstration selector mode: 'fixed', 'dynamic', while got {mode}"
        self.mode = mode
        self.k = k
        examples = self.load_examples(self, demo_dir_or_file)
        self.examples = self.fit_domain(self, examples, domain)
    ...
    def load_examples(self, dir: str) -> List[Dict]:
        ...
        assert len(
            examples) > 0, ("Failed to load examples. Note that only .jsonl file format is supported for demonstration "
                            "loading.")
        return examples
...
if __name__ == "__main__":
    selector = DemoSelector("dynamic",
                            "./demonstration/seed/", k=3, domain="beauty_product")
