...
class UserProfileMemory:
    ...
    def conclude_user_profile(self, conversation: str) -> str:
        ...
        prompt += (f"The profile consists of three parts: history, like and unwanted.Each part is a list. You should "
                   f"return a json-format string.\nHere are some examples.\n{_FEW_SHOT_EXAMPLES}\nNow extract user "
                   f"profiles from below conversation: \n> Conversation\n{conversation}\n> Profiles\n")
    ...
    def correct_format(self, err_resp: str) -> str:
        prompt = ("Your task is to correct the string to json format. Here are two examples of the format:\n{"
                  "\"history\": [\"ITEM-1\", \"ITEM-2\", \"ITEM-3\"], \"like\": [\"ITEM-100\"], \"unwanted\": ["
                  "\"ITEM-17\", \"ITEM-19\", \"ITEM-30\"]}\nThe string to be corrected is {err_resp}. It can not be "
                  "parsed by Python json.loads(). Now give the corrected json format string.").replace(
            "{err_resp}", err_resp)
        return self.llm_engine.call(
            user_prompt=prompt,
