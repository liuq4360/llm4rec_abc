...
from utils.util import get_topk_index, normalize_np
    ...
    def run(self, inputs: str):
        ...
        try:
            ...
            if schema not in {"popularity", "preference", "similarity"}:
                info += (f"{self.name}: ranking schema switch to 'popularity' because only ['popularity', "
                         f"'preference', 'similarity'] are supported but get {schema}. \n")
                schema = "popularity"

            similarity_score = self.buffer.similarity
            if schema == "similarity" and (similarity_score is None):
                schema = "popularity"
                info += (f"{self.name}: ranking schema switch to 'popularity' because similarity scores have not been "
                         f"calculated with Similarity Filtering Tool. \n")
            ...
            if schema == "popularity":
                ...
                info += (f"{self.name}: Items are ranked according to their popularises. The ranked game ids are "
                         f"stored in buffer and are visible to all tools. \n")
                ...
                self.buffer.push(self.name, rec_id)
                info += (f"{self.name}: Items are ranked according to the similarity with seed items. The ranked game "
                         f"ids are stored in buffer and are visible to all tools. \n")
                ...
                info += ("{self.name}: Items are ranked according to the preference. The ranked game ids are stored in "
                         "buffer and are visible to all tools. \n")
