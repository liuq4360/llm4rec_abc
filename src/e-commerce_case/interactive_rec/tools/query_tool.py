...
class QueryTool:
    ...
    def __init__(self, name: str, desc: str, item_corpus: BaseGallery, buffer, result_max_token: int = 512) -> None:
        ...
        self._max_record_num = self.result_max_token // 5  # each record at least 5 tokens. If too many records,
        # sample randomly
    ...
    def run(self, inputs: str) -> str:
        ...
        try:
            ...
            info += (f"{self.name}: The input SQL is rewritten as {inputs} because "
                     f"some {self.item_corpus.categorical_col_values.keys()} are not existing. \n")
        except:
        ...
        try:
            ...
            if num_tokens_from_string(json.dumps(res)) > self.result_max_token:
                ...
                for i, record in enumerate(res):
                    ...
                    # if not cut last time, the loop would end due to the cut operation in the loop
                    # would not shorten the res anymore.
